...
function priceRow(row, rates) {
  ...
  if ((category === 'through' || category === 'over') && row.sqm !== null) {
    ...
    const { label, rate, overMax } = bracketLookup(row.sqm, table);
    unitCost = rate;
    totalCost = rate * qty;
    flag = overMax;
    ...
    note = `${row.size1}mm x ${row.size2}mm batt & intumescent mastic, ${orientation} `
      + `(${row.sqm.toFixed(4)}m\u00b2, rounds up to ${label} bracket) \u2013 \u00a3${rate} each`
      + (qty > 1 ? ` x${qty} = \u00a3${totalCost}` : '');
    if (overMax) note += ' \u2013 EXCEEDS 1m2 max bracket, rate capped at 1m2 rate, please review';
